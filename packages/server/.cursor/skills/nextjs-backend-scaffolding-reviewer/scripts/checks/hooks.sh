...
check_hooks() {
  ...
  for f in "$use_list" "$use_suspense" "$use_create" "$use_update" "$use_delete"; do
    require_grep "$f" '^"use client"' "$(basename "$f") must start with \"use client\" (first line)" || true
  done

  if grep -q '"use client"' "$hydrate" 2>/dev/null; then
    reasons+=("$(basename "$hydrate") must be a server component — must not contain \"use client\"")
  fi
