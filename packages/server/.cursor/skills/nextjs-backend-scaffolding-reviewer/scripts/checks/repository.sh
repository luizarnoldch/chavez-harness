...
# default-import the client as `db` from "@/lib/db" -- there is no `prisma` import
# from "@/lib/prisma" anywhere in this project's templates. Keep this check
# pinned to default `import db from "@/lib/db"` (src/lib/db.ts is default-only).
check_repository() {
  ...
  require_grep "$file" 'import db from "@/lib/db"' 'must import db from "@/lib/db" (default import; not named { db })' || true
