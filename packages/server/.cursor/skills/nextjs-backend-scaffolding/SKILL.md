...
If the user or orchestrator provides a `features/tasks/prd-*.md` or `features/tasks/ticket-*.md` path:

1. Prefer `<repo-root>/.cursor/skills/prd-authoring/scripts/extract-execution.sh`.
2. Run: `bash <extract-execution.sh> <file> --skill=nextjs-backend-scaffolding`
3. Use JSON `target` plus `params` (`entity`, `layers`, `transport`, `database`) as CLI flags. Do not invent values already present.
4. If that script is missing, read the document's `## Execution` YAML fence and take the `skills[]` entry whose `name` is `nextjs-backend-scaffolding`.
5. If a required param is missing, ask. If no PRD/ticket path is given, follow the decision flow below.
