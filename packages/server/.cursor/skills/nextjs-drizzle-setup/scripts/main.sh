#!/usr/bin/env bash
# Orchestrate Drizzle foundation install: deps → files → scripts → generate → validate.
# Usage: main.sh --root <project> [--force] [--force-install] [--skip-deps] [--files-only] [--skip-generate]
set -euo pipefail
...
show_help() {
  cat <<EOF
Next.js Drizzle foundation setup (literal assets)
...
Options:
  ...
  --files-only        Copy assets + patch scripts only (skip generate)
  --skip-generate     Skip bunx drizzle-kit generate
