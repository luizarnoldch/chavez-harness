#!/usr/bin/env bash
# Ensure canonical Drizzle-related packages are in the target package.json via bun.
...
run_canonical_adds() {
  echo "Running: bun add drizzle-orm@rc pg dotenv"
  (cd "$ROOT" && bun add drizzle-orm@rc pg dotenv)
  echo "Running: bun add -D drizzle-kit@rc tsx @types/pg"
  (cd "$ROOT" && bun add -D drizzle-kit@rc tsx @types/pg)
}
