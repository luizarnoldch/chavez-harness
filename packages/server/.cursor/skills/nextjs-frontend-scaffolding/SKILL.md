...
2. Run: `bash <extract-execution.sh> <file> --skill=nextjs-frontend-scaffolding`
3. Use JSON `target` plus `params` (`entity`, `flag`, `transport`) as CLI flags. Do not invent values already present.
4. If that script is missing, read the document's `## Execution` YAML fence and take the `skills[]` entry whose `name` is `nextjs-frontend-scaffolding`.
5. If a required param is missing, ask. If no PRD/ticket path is given, follow Path resolution and Rules below.
