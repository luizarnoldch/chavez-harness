...
const [Entity]List = () => {
  ...
  return (
    <div className="grid gap-4" data-cy-[entity-kebab]-list="[entity-kebab]-list">
      <div className="flex items-center justify-between">
        <span className="text-sm text-gray-500">{[entity]s.length} [entity]s</span>
        <button
          type="button"
          data-cy-submit-create-[entity-kebab]-btn="create-[entity-kebab]-btn"
        ...
        <table className="w-full border-collapse text-sm">
          ...
          <tbody>
            {[entity]s.map(([entity]) => (
              <tr
                key={[entity].id}
                data-cy-[entity-kebab]-row="[entity-kebab]-row"
                data-cy-[entity-kebab]-id={[entity].id}
                className="border-b hover:bg-gray-50"
              >
                {/* Add <td> for each schema field; title cell needs data-cy-[entity-kebab]-title */}
                <td className="px-3 py-2 font-mono text-xs text-gray-500">{[entity].id}</td>
                <td className="px-3 py-2 text-right">
                  <div className="flex justify-end gap-3">
                    <button
                      type="button"
                      data-cy-submit-edit-[entity-kebab]-btn="[entity-kebab]-edit-btn"
                    ...
                    <button
                      type="button"
                      data-cy-submit-delete-[entity-kebab]-btn="[entity-kebab]-delete-btn"
