#!/usr/bin/env bash
# Copy canonical Cypress assets into project root.
...
while IFS='|' read -r asset_rel dest_rel; do
  ...
  if [[ -e "$dest" ]] && [[ "$FORCE" != true ]]; then
    ...
    # selectors may already include entity blocks from nextjs-cypress-e2e
    if [[ "$dest_rel" == "cypress/support/selectors.ts" ]]; then
      echo "SKIP (selectors differs; use --force to replace auth-only base): $dest_rel"
      skipped=$((skipped + 1))
      continue
    fi
