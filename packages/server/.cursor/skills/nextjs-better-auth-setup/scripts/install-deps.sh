#!/usr/bin/env bash
# Ensure better-auth is in the target package.json via bun.
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --help|-h)
      ...
      echo "Installs better-auth with bun (see reference/deps.md)."
      exit 0
      ;;
    *)
...
run_canonical_adds() {
  echo "Running: bun add better-auth"
  (cd "$ROOT" && bun add better-auth)
}
