...
var shellArgs = (str) => {
  ...
  for (let i = 0; i < str.length; ++i) {
    ...
    if (/\s/.test(char)) {
      if (current !== void 0) args.push(current);
      current = void 0;
      continue;
    }
...
var clone = (value, seen = /* @__PURE__ */ new WeakMap()) => {
  ...
  if (Array.isArray(value)) {
    ...
    for (let i = 0; i < value.length; ++i) {
      if (i in value) copy2[i] = clone(value[i], seen);
    }
    copy2.length = value.length;
    return copy2;
  }
  if (value instanceof Map) {
    const copy2 = /* @__PURE__ */ new Map();
    seen.set(value, copy2);
    for (const [k, v] of value) copy2.set(clone(k, seen), clone(v, seen));
    return copy2;
  }
  if (value instanceof Set) {
    const copy2 = /* @__PURE__ */ new Set();
    seen.set(value, copy2);
    for (const v of value) copy2.add(clone(v, seen));
    return copy2;
  }
...
var commandsInfo = (commands) => {
  ...
  return Object.fromEntries(validated.map((c) => [c.name, {
    ...
    isHidden: c.hidden,
    options: c.options ? Object.fromEntries(Object.entries(c.options).map(([key, opt]) => [key, clone(opt.config)])) : void 0,
