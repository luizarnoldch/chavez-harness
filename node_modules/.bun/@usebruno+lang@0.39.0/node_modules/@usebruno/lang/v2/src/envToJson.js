...
const {
  ...
  SECRET_PROVIDER_IDENTIFIER_FIELDS
} = require('./utils');
...
const sem = grammar.createSemantics().addAttribute('ast', {
  ...
  externalsecrets(_1, name, dictionary) {
    ...
    const variables = mapPairListToKeyValPairs(dictionary.ast)
      // Keep any named entry, even one with an empty identifier — a half-filled secret row is the
      // user's data; dropping it here would silently delete it on the next save. (Unnamed entries
      // are not real rows.)
      .filter((pair) => pair && pair.name)
      .map((pair) => {
        const entry = {
          name: pair.name,
          [identifierField]: typeof pair.value === 'string' ? pair.value : '',
          enabled: pair.enabled !== false
        };
        if (pair.annotations) {
          entry.annotations = pair.annotations;
        }
        return entry;
      });
