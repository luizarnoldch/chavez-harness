...
`;
  ...
  it('should parse a vars:externalsecrets block into nested externalSecrets, mapping the identifier by provider type', () => {
    const input = `
vars {
  url: http://localhost:3000
  ~port: 3000
...
vars:externalsecrets:azure-key-vault {
  azureSecret: my-key-vault
  ~dbPassword: secret/data/db-password
}
`;
    ...
    const expected = {
      variables: [
        ...
        { name: 'port', value: '3000', enabled: false, secret: false }
      ],
      externalSecrets: {
        type: 'azure-key-vault',
        variables: [
          { name: 'azureSecret', vaultName: 'my-key-vault', enabled: true },
          { name: 'dbPassword', vaultName: 'secret/data/db-password', enabled: false }
        ]
      }
    };
  ...
  it('should map the identifier to projectId for gcp, one row per project', () => {
    const input = `
vars:externalsecrets:gcp-secrets-manager {
...
`;
    ...
    expect(parser(input)).toEqual({
      externalSecrets: {
        ...
        variables: [
          { name: 'staging', projectId: 'my-staging-project', enabled: true },
          { name: 'prod', projectId: 'my-prod-project', enabled: false }
        ]
      }
    });
  });

  it('should map the identifier to path for hashicorp providers', () => {
    const input = `
vars:externalsecrets:hashicorp-vault-server {
  db: secret/data/database
}
`;
    ...
    expect(output.externalSecrets.variables).toEqual([
      { name: 'db', path: 'secret/data/database', enabled: true }
    ]);
  });

  it('should preserve a named entry with an empty identifier value', () => {
    const input = `
vars:externalsecrets:hashicorp-vault-server {
  good: secret/data/good
  bad:
}
`;
    ...
    const output = parser(input);
    expect(output.externalSecrets.type).toBe('hashicorp-vault-server');
    // A half-filled row is the user's data — keep it (with an empty path) rather than silently
    // dropping it, which would delete it on the next save.
    expect(output.externalSecrets.variables).toEqual([
  ...
  it('should produce an empty variables array for a block with no entries', () => {
    const input = `
...
`;
  ...
  it('should ignore a block with an unknown provider type', () => {
    const input = `
vars:externalsecrets:unknown-provider {
  orphan: secret/data/orphan
}
`;
