...
const { recordLegacySecretSyntaxUsage } = require('../utils/external-secrets');
...
const prepareRequest = async (item = {}, collection = {}) => {
  ...
  // Record legacy {{$secrets.*}} usage off the merged request config, where the markers are still raw:
  // the runner interpolates them away later, and the returned axiosRequest URL-encodes query-param
  // secrets and inlines file bodies, both of which would defeat the scan.
  recordLegacySecretSyntaxUsage(request);
